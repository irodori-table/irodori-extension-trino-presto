...
fn connect(request: &Value) -> IrodoriConnectorBuffer {
    ...
    let config =
        match runtime().and_then(|runtime| runtime.block_on(TrinoConfig::from_request(request))) {
            Ok(config) => config,
...
mod tests {
    ...
    fn block_on_config(request: &Value) -> Result<TrinoConfig, String> {
        runtime()
            .expect("runtime")
            .block_on(TrinoConfig::from_request(request))
    }
